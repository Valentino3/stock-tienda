...
const badgeVariants = cva(
  // `rounded-md` y no píldora: una píldora se lee como etiqueta que alguien
  // puso, y estos son estados de una fila. Cuadrada y apretada entra en una
  // fila de tabla sin empujarla, que es donde vive el 90% de estos badges.
  "group/badge inline-flex h-5 w-fit shrink-0 items-center justify-center gap-1 overflow-hidden rounded-md border border-transparent px-1.5 py-0.5 text-xs font-medium whitespace-nowrap transition-all focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 has-data-[icon=inline-end]:pr-1 has-data-[icon=inline-start]:pl-1 aria-invalid:border-destructive aria-invalid:ring-destructive/20 [&>svg]:pointer-events-none [&>svg]:size-3!",
  {
    variants: {
      variant: {
        ...
        brand:
          "bg-brand-muted text-brand focus-visible:ring-brand/30 [a]:hover:bg-brand/20",
        outline:
